...
// Load the controller
void GazeboRosDiffDriveLowLevel::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  this->parent = _parent;
  ...
  gazebo_ros_->getParameter<std::string>(command_topic_, "commandTopic", "/motors");
  ...
  gazebo_ros_->getParameter<std::string>(robot_base_frame_, "robotBaseFrame",
                                         "base_footprint");
  ...
  gazebo_ros_->getParameterBoolean(publishWheelEncoders_, "publishWheelEncoders", false);
  gazebo_ros_->getParameterBoolean(publishWheelJointState_, "publishWheelJointState",
                                   false);
  ...
  gazebo_ros_->getParameter<unsigned>(encoder_resolution_, "encoderResolution", 2400);
  gazebo_ros_->getParameter<double>(wheel_torque, "wheelTorque", 5.0);
  gazebo_ros_->getParameter<double>(update_rate_, "updateRate", 100.0);
  std::map<std::string, OdomSource> odomOptions;
  odomOptions["encoder"] = ENCODER;
  odomOptions["world"] = WORLD;
  gazebo_ros_->getParameter<OdomSource>(odom_source_, "odometrySource", odomOptions,
                                        WORLD);
  ...
  if(this->publishWheelJointState_)
  {
    joint_state_publisher_ =
      gazebo_ros_->node()->advertise<sensor_msgs::JointState>("joint_states", 1000);
  ...
  ros::SubscribeOptions so = ros::SubscribeOptions::create<snd_msgs::Motors>(
    command_topic_, 1, boost::bind(&GazeboRosDiffDriveLowLevel::cmdVelCallback, this, _1),
    ros::VoidPtr(), &queue_);
...
#if GAZEBO_MAJOR_VERSION < 8
    const double angle = joint->GetAngle(0).Randians;
#else
    const double angle = joint->Position(0);
#endif
...
#if GAZEBO_MAJOR_VERSION < 8
    const math::Pose poseWheel = joints_[i]->GetChild()->GetRelativePose();
...
#else
    const ignition::math::Pose3d poseWheel = joints_[i]->GetChild()->RelativePose();
    tf::Quaternion qt(poseWheel.Rot().X(), poseWheel.Rot().Y(), poseWheel.Rot().Z(),
                      poseWheel.Rot().W());
    tf::Vector3 vt(poseWheel.Pos().X(), poseWheel.Pos().Y(), poseWheel.Pos().Z());
#endif
...
#else
  const double angle_l = joints_[LEFT]->Position(0);
  const double angle_r = joints_[RIGHT]->Position(0);
#endif
...
#if GAZEBO_MAJOR_VERSION < 8
  const common::Time current_time = parent->GetWorld()->GetSimTime();
#else
  const common::Time current_time = parent->GetWorld()->SimTime();
#endif
...
void GazeboRosDiffDriveLowLevel::cmdVelCallback(const snd_msgs::Motors::ConstPtr& cmd_msg)
{
  boost::mutex::scoped_lock scoped_lock(lock);
  if(pwm_input_)
  {
    vel_cmd_[LEFT] =
      (left_pwm_lut_.InterpolateLinear(static_cast<double>(cmd_msg->left)) * 2 * M_PI) /
      encoder_resolution_;
    vel_cmd_[RIGHT] =
      (right_pwm_lut_.InterpolateLinear(static_cast<double>(cmd_msg->right)) * 2 * M_PI) /
      encoder_resolution_;
...
{
  const double vl = joints_[LEFT]->GetVelocity(0);
  const double vr = joints_[RIGHT]->GetVelocity(0);
#if GAZEBO_MAJOR_VERSION < 8
  const common::Time current_time = parent->GetWorld()->GetSimTime();
#else
  const common::Time current_time = parent->GetWorld()->SimTime();
  ...
  const double sl = vl * (wheel_diameter_ / 2.0) * seconds_since_last_update;
  const double sr = vr * (wheel_diameter_ / 2.0) * seconds_since_last_update;
  const double ssum = sl + sr;

  const double sdiff = sr - sl;

  const double dx = (ssum) / 2.0 * cos(pose_encoder_.theta + (sdiff) / (2.0 * b));
  const double dy = (ssum) / 2.0 * sin(pose_encoder_.theta + (sdiff) / (2.0 * b));
  const double dtheta = (sdiff) / b;

  pose_encoder_.x += dx;
  pose_encoder_.y += dy;
  pose_encoder_.theta += dtheta;

  const double w = dtheta / seconds_since_last_update;
...
#else
    const ignition::math::Pose3d pose = parent->WorldPose();
    qt = tf::Quaternion(pose.Rot().X(), pose.Rot().Y(), pose.Rot().Z(), pose.Rot().W());
    vt = tf::Vector3(pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z());
#endif
...
#if GAZEBO_MAJOR_VERSION < 8
    const math::Vector3 linear = parent->GetWorldLinearVel();
    odom_.twist.twist.angular.z = parent->GetWorldAngularVel().z;

    // convert velocity to child_frame_id (aka base_footprint)
    const float yaw = pose.rot.GetYaw();
    odom_.twist.twist.linear.x = cosf(yaw) * linear.x + sinf(yaw) * linear.y;
    odom_.twist.twist.linear.y = cosf(yaw) * linear.y - sinf(yaw) * linear.x;
#else
    const ignition::math::Vector3d linear = parent->WorldLinearVel();
